...
class FeatureEngineering:
    ...
    def bin_numerical_feature(
        self, cols: str, bins: list, labels: list
    ) -> pd.DataFrame:
        """This method bins the numerical columns into specified
        bins and labels them accordingly"""
        self.df[str(cols) + "_binned"] = pd.cut(
            self.df[cols], bins=bins, labels=labels, include_lowest=True
        )
        return self.df
    ...
    def engineer_family_size(
        self, col: str, value: str, new_col_name: str, bins: list, labels: str
    ) -> pd.DataFrame:
    ...
    def engineer_marital_status(
        self, col: str, value: str, grouping_dict: dict
    ) -> pd.DataFrame:
    ...
    def standardise_feature(
        self, cols: list, mldatatype: str, path: str
    ) -> pd.DataFrame:
...
def main(input_filepath: str, output_filepath: str, ml_datatype: str) -> None:
    ...
    marital_dict = {
        ...
        "Widowed": [
            ...
            "Divorcedwidowed",
            "Widowedwidowed",
            "Living_With_Partnerwidowed",
        ],
        "Not Supplying": ["Not Disclosed"],
        "civil_partnership": [
            "Living_With_Partner",
            "Singleliving_With_Partner",
            "Marriedliving_With_Partner",
            "Living_With_Partnerliving_With_Partner",
            "Widowedliving_With_Partner",
            "Divorcedliving_With_Partner",
        ],
    }
    ...
    example.engineer_marital_status(
        col="marital_status", value="Not disclosed", grouping_dict=marital_dict
    )
    example.bin_numerical_feature(
        "age", bins=[0, 25, 50, 125], labels=["young", "middle_aged", "old"]
    )
    example.bin_numerical_feature(
        "income", bins=[0, 25000, 50000, 200000], labels=["low", "medium", "high"]
    )
    example.engineer_family_size(
        col="no_of_children",
        value=0,
...
if __name__ == "__main__":
    ...
    try:
        ...
        for data_type in dataset:
            ...
            logger.info(
                ">>>> {data_type} data has been transformed and saved \
                successfully %s",
                data_type,
            )
        logger.info(
            ">>>>> Ended process for feature engineering <<<<< %s", PROJECT_STAGE
        )
